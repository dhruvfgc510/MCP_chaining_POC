...
def _redact_parsed_payload_for_log(data: Dict[str, Any]) -> Dict[str, Any]:
    """Return a copy of a parsed dict with sensitive keys redacted."""
    return {k: "<redacted>" if k in _SENSITIVE_LOG_KEYS else v for k, v in data.items()}
...
def play(input_data: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warning("Invalid 'remaining_attempts' value: %r (%s)", raw_remaining, exc)
